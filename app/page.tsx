...
import { BREAD_RECIPES, type BreadType } from "@/lib/bread-types"
...
// Rendered twice in JSX for seamless infinite loop
const MARQUEE_ITEMS = [
  "אני לא אלים כי אני על עלים, אבל תנסה להתעסק איתי אתה תגמור כמו בצק עלים - שבק ס",
  "באגט", "פוקצ'ה", "מדריך מחמצת", "ואחרי שהכול ייגמר תישאר רק אהבה פשוטה פשוטה פשוטה כמו לחם טובה כמו אהבה - יאיר זיו", "קמחים",
  "אני מוכן לחיות על לחם ועל מים ורק לראות אותך תמיד מול העיניים - סטלוס ואורן חן",
  "חלה", "קופידון ירה בי חץ והתאהבתי בחמץ - אורטגה", "Rise & Crumb", "אני זוכר אותה קונה שם לחם קימל, אני זוכר אותה מזמן מכיתה גימל - כוורת",
  "היי אחי, החיים שלך בלאפה, היי אחי, זורק את הכל לפיתה גדולה - טיפקס",
]
...
function BreadCarousel() {
  ...
  return (
    <div className="float-animate relative">
      ...
      {/* Circle with crossfading images */}
...
/* ─── Page ─── */
export default function RiseAndCrumb() {
  const { t } = useLanguage()
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ─── MARQUEE ─── */}
      {/* dir="ltr" forces the strip to start from the left so translateX(-50%) loops correctly */}
      ...
      <div id="recipes" className="max-w-4xl mx-auto px-6 py-10">
        ...
        <main>
          <div className="space-y-3">
            {(Object.entries(BREAD_RECIPES) as [BreadType, typeof BREAD_RECIPES[BreadType]][]).map(
              ([type, config], index) => (
